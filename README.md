...
3. **`read`** — each selected URL is fetched with `httpx` and cleaned with BeautifulSoup (scripts, nav, footers stripped; text capped at 8 KB per source).
...
The suite covers the state machine (phase transitions, trace aggregation, cost math). The agent loop, tools, and WebSocket protocol are not covered yet.
